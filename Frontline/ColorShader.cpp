...
bool ColorShaderClass::Initialize(ID3D11Device* device, HWND hwnd) {
	...
	result = InitializeShader(device, hwnd, L".\\Data\\Shaders\\ColorVertexShader.hlsl", L".\\Data\\Shaders\\ColorPixelShader.hlsl");
	if (!result) {
		return false;
	}
...
bool ColorShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
	...
	if (FAILED(result)) {
		...
		if (errorMessage) {
			...
			// If nothing was in the error message the shader was missing.
