...
bool InstancedTexturedRect::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext,
	...
	m_shaderType = H_2D_TEXTURE_RTYPE;
...
bool InstancedTexturedRect::Render(ID3D11DeviceContext* deviceContext, int positionX, int positionY) {
	bool result;
	m_posX = positionX;
	m_posY = positionY;
...
bool InstancedTexturedRect::Render(ID3D11DeviceContext* deviceContext, int positionX, int positionY, DirectX::XMFLOAT4 color) {
	bool result;
	m_posX = positionX;
	m_posY = positionY;
	// Re-build the dynamic vertex buffer for rendering to possibly a different location on the screen.
...
bool InstancedTexturedRect::UpdateBuffers(ID3D11DeviceContext* deviceContext, int positionX, int positionY, DirectX::XMFLOAT4 color) {
...
bool InstancedTexturedRect::UpdateBuffers(ID3D11DeviceContext* deviceContext, int positionX, int positionY) {
	float left, right, top, bottom;
...
void InstancedTexturedRect::RenderBuffers(ID3D11DeviceContext* deviceContext) {
	...
	switch (m_shaderType) {
	case H_2D_COLOR_SHADERTYPE:
		stride[0] = sizeof(H_2D_COLOR_RESOURCETYPE);
		break;
	case H_2D_TEXTURE_RTYPE:
		stride[0] = sizeof(H_2D_TEXTURE_RESOURCETYPE);
		break;
	case H_2D_COLOR_TEXTURE_SHADERTYPE:
		stride[0] = sizeof(H_2D_COLOR_TEXTURE_RESOURCETYPE);
		break;
	}
...
void InstancedTexturedRect::SetShaderType(int type) {
	m_shaderType = type;
}