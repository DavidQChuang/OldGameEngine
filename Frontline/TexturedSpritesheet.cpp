...
void TexturedSpritesheet::SetSprite(ID3D11DeviceContext* deviceContext, int s) {
	...
	case H_2D_TEXTURE_SHADERTYPE:
		// Create the vertex array.
		H_2D_COLOR_TEXTURE_RESOURCETYPE* texturetype = new H_2D_COLOR_TEXTURE_RESOURCETYPE[m_vertexCount];
		if (!texturetype) {
			return;
		}
		// Load the vertex array with data.
		...
		H_2D_TEXTURE_RESOURCETYPE* texturePtr = (H_2D_TEXTURE_RESOURCETYPE*)mappedResource.pData;

		// Copy the data into the vertex buffer.
		memcpy(texturePtr, (void*)texturetype, (sizeof(H_2D_COLOR_TEXTURE_RESOURCETYPE) * m_vertexCount));
		delete[] texturetype;
		texturetype = 0;
		break;
	case H_2D_COLOR_TEXTURE_SHADERTYPE:
